...
class Controller(Node):
    ...
    def __init__(self, name):
        ...
        if 'Mecanum' in self.machine_type:
            ...
            # ROVAC: tread diameter=42mm, tread separation=155mm
            # wheelbase+track_width must equal tread separation for diff-drive math
            self.mecanum = mecanum.MecanumChassis(wheelbase=0.0, track_width=0.155, wheel_diameter=0.042)
    ...
    def load_calibrate_param(self, request, response):
        if 'Tank' in self.machine_type:
            self.linear_factor = self.get_parameter('~linear_correction_factor_tank').value or 1.00
        else:
        ...
        if 'Acker' not in self.machine_type:
            ...
            if msg.angular.z != 0:
                ...
                if speeds[0] is not None:
                    # self.get_logger().info('\033[1;32m%s\033[0m' % int(speeds[0]))
