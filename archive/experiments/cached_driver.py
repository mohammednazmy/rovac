#!/usr/bin/env python3
"""
ROS2 Tank Motor Driver - State Caching Version
Only writes to GPIO when values actually change to prevent PWM jitter/reset issues.
"""
...
class TankMotorDriver(Node):
    def __init__(self):
        ...
        self.speed_percent = 70
        self.pwm_freq = 1000
        self.gpio_handle = None
        
        # State Cache: (direction, pwm_val)
        # direction: 1=fwd, -1=bwd, 0=stop
        self.last_left_state = (None, None)
        self.last_right_state = (None, None)
        ...
        if GPIO_AVAILABLE:
            try:
                ...
                self.get_logger().info('GPIO initialized - State Caching Enabled')
        ...
        self.get_logger().info('Tank motor driver ready (State Caching)')
    ...
    def set_motor(self, side, direction, pwm_val):
        ...
        # Check against cache
        new_state = (direction, pwm_val)
        if side == 'left':
            if self.last_left_state == new_state:
                return # No change, do nothing
            self.last_left_state = new_state
            pwm_pin = self.ENA
            in1 = self.IN1
            in2 = self.IN2
        else:
            if self.last_right_state == new_state:
                return # No change, do nothing
            self.last_right_state = new_state
            pwm_pin = self.ENB
            in1 = self.IN3
            in2 = self.IN4
