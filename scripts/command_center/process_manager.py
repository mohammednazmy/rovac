...
class ProcessManager:
    ...
    def dump_diagnostics(self, ros_bridge=None, callback=None,
        ...
        def worker():
            ...
            # ── 0. TUI state (which tab, Drive panel state) ──────────
            if ui_state:
                lines = []
                lines.append(f"  active_tab: {ui_state.get('active_tab')}")
                drive = ui_state.get('drive')
                if isinstance(drive, dict):
                    lines.append("  Drive panel state:")
                    for k, v in drive.items():
                        lines.append(f"    {k}: {v}")
                else:
                    lines.append(f"  Drive panel: {drive}")
                sections.append(_section('TUI state', '\n'.join(lines)))
            ...
            # ── 6b. /cmd_vel_teleop publishers — answer "who is the zombie?"
            sections.append(_section(
                '/cmd_vel_teleop publishers (zombie hunt)',
                _run("source ~/robots/rovac/config/ros2_env.sh 2>/dev/null;"
