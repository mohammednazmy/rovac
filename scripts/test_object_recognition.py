#!/usr/bin/env python3
"""
Test script for object recognition node
"""

import sys
import os

# Add the robot_mcp_server directory to Python path
sys.path.append(os.path.join(os.path.dirname(__file__), "..", "robot_mcp_server"))


def test_imports():
    """Test if all required modules can be imported"""
    try:
        import rclpy

        print("✓ rclpy imported successfully")
    except ImportError as e:
        print(f"✗ rclpy import failed: {e}")

    try:
        import cv2

        print("✓ cv2 imported successfully")
        print(f"  OpenCV version: {cv2.__version__}")
    except ImportError as e:
        print(f"✗ cv2 import failed: {e}")

    try:
        import numpy as np

        print("✓ numpy imported successfully")
    except ImportError as e:
        print(f"✗ numpy import failed: {e}")

    try:
        from cv_bridge import CvBridge

        print("✓ cv_bridge imported successfully")
    except ImportError as e:
        print(f"✗ cv_bridge import failed: {e}")

    try:
        # Try to import our node
        import object_recognition_node

        print("✓ object_recognition_node imported successfully")
    except ImportError as e:
        print(f"✗ object_recognition_node import failed: {e}")


if __name__ == "__main__":
    print("Testing Object Recognition Node Imports")
    print("=" * 40)
    test_imports()
